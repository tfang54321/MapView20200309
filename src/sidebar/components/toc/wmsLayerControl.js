...
const dom_Row_height=30; // HEIGHT OF DOM ROW FOR AUTOSIZER 
...
export function getWMSTileLayer(serverURLs, layerName,sourceInfor=null,ext=null) 
{
  let olLayer=null;
  let olSource=null;
  if(sourceInfor && sourceInfor.length>0){
    let sourceName =null;
    let attribs=null;
    ...
    olLayer= new OlTileLayer({
         ...
         // extent: ext!==null?ext:null,
          source: olSource//,
          // minResolution: minScaleDenominator,
          // maxResolution: maxScaleDenominator
      });
    }
    else {
      olSource =  new OlTileWMSSource({
...
function getSourceValue(sourceInfor,key)
{
  if(sourceInfor && sourceInfor.length >0) {
    for(let i in sourceInfor) {
      if(String(sourceInfor[i].key).toLocaleLowerCase() ===String(key).toLocaleLowerCase()){
        return sourceInfor[i].value;
      }
    }
  }
  return null;
...
function createWMSBottomLayer(parentLayer,layer,sourceInfor,commonInfor,outputList)
{
    ...
    const minScale = layer.MinScaleDenominator===undefined?null:layer.MinScaleDenominator;
    const maxScale = layer.MaxScaleDenominator===undefined?null:layer.MaxScaleDenominator;
    ...
    if(window.appConfig.CreateBottomLayerInOL) 
    //if(1) 
    {
      keepIt=false;
      newOLLayer =null// getWMSTileLayer(wmsurl,layerNameOnly,sourceInfor);
      if(newOLLayer!==undefined) {
        //newLayer.setVisible(!group.defaultGroup ? false : layerVisible);
        //"setOpacity" image layer only
        //newOLLayer.setOpacity(opacity);
        newOLLayer.setProperties({ name: layerNameOnly, displayName: layerTitle });
        newOLLayer.setZIndex(layerZIndex);
        //added into the open layer map for display
        //BLI: not add the layer to map here (only the user want to control bottom feature seperately )
        //window.map.addLayer(newOLLayer);
        //addToMap=true;
        keepIt=true;
      }
    }
    if(keepIt) {
      ...
      const returnLayer = {
          ...
          liveLayer: null,// liveLayer, // LIVE LAYER FLAG
          wfsUrl:null,//wmsurl, // BLI: used to requery the feature information (meta data)
          displayName: layerTitle, // DISPLAY NAME USED BY IDENTIFY and dom list row
          addedToMap: addToMap, // flag to determine if the layer has been added into open layer map
  ...
  if(layerContainer){
    if($.type(layerContainer)==='object') {
      if(layerContainer.leaf){
        ...
        //let openLayer= getWMSTileLayer(urls, layerContainer.name,null,layerContainer.geoExt);
        let newContainer ={
...
function createWMSTopLayerObj(topLayer,allLayers,urls,wmsGetCapaUrl)
{
  ...
  if(allLayers && layer &&urls &&wmsGetCapaUrl )
  {
    ...
    //create a layer to hold the display object of openlayers
    let olLayerObj=getWMSTileLayer(urls,name);
        ...
        if(urls.length<1) {
          ...
          alert("No WMS layer available!");
            ...
            if(topLayer && topLayer!== undefined) {
              ...
              //create super list to contain All feature layers:
...
function wmsTopLayer_updateVis(curLayer,topLayer)
{
   if(curLayer && topLayer) {
    ...
    if(parent && parent.olLayer!==null)
    {
      ...
      if(layerList===undefined) {
        if(visible===true && source) {
          ...
          if(parent.addedToMap=== false)
          {
            ...
            window.map.addLayer(parent.olLayer.olLayer);
          }
        }
        return;
      }
      ...
      if(updateParent) {
        ...
        {
          parent.olLayer.olLayer.setVisible(false);
          //Because the current implemenation is using "zIndex" to control the layer display
          //priority, we can just remove the layer from map directly.
          window.map.removeLayer(parent.olLayer.olLayer);
          parent.olLayer.olSource.clear();
...
export function turnOffLayers(layers, callback) {
  for (let index = 0; index < layers.length; index++) {
    ...
    if(layer.visible && (layer.layer && layer.layer !==undefined)){
      //The "layer.layer" of bottom feature will not hold the ol source object.
      if(layer.containOLSource) {
          layer.layer.olLayer.setVisible(false);//turn off the ol layer
      }
      else {
        layer.layer.setVisible(false);//turn off the ol layer
      }
    }
    ...
    if(layer.visible){
      ...
      let topLayer=newLayer.parentLayer
      while (topLayer)
      {
      ...
      if(topLayer===undefined && isWMSTopLayer(newLayer))
        topLayer =newLayer;
      wmsTopLayer_updateVis(newLayer,topLayer);
    }
...
export function enableLayersVisiblity(layers, callback) {
  for (let index = 0; index < layers.length; index++) {
    ...
    if(!(layer.visible) && layer.layer && layer.layer !==undefined){
        //The "layer.layer" of bottom feature will not hold the ol source object.
      if(layer.containOLSource) {
        layer.layer.olLayer.setVisible(true);//turn off the ol layer
      }
      else {
        layer.layer.setVisible(true);//turn off the ol layer
      }
    } 
    let newLayer = Object.assign({}, layer);
