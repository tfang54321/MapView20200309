...
const sidebar_tabIndexValue={layers:0,mapview:1,mymaps:2,themes:3,reports:4}; 
// TEST COMMENT
class Sidebar extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      // COMPONENTS (BLI: used to inital the complex component.)
      activeTabComponents: {
       // mapview: <MapView key={helpers.getUID()} />,
  ...
  getTabIndex=tabName=>{
    let index=-1;
    if (String(tabName).toLowerCase() === "layers") {
      index= sidebar_tabIndexValue.layers;}
    else if (String(tabName).toLowerCase() === "mapview") {
      index= sidebar_tabIndexValue.mapview;}
    else if (String(tabName).toLowerCase() === "tools") {
      index= sidebar_tabIndexValue.tools;}
    else if (String(tabName).toLowerCase() === "mymaps") {
      index= sidebar_tabIndexValue.mymaps;}
    else if (String(tabName).toLowerCase() === "themes") {
      index= sidebar_tabIndexValue.themes;}
    else if (String(tabName).toLowerCase() === "reports") {
      index= sidebar_tabIndexValue.reports;}
    return index;
  }

  getTabName=index=>{
    let tabName='';
    if (index=== sidebar_tabIndexValue.layers) tabName = "Layers" ;
    else if (index=== sidebar_tabIndexValue.mapview) tabName = "MapView";
    else if (index=== sidebar_tabIndexValue.tools) tabName = "Tools";
    else if (index=== sidebar_tabIndexValue.mymaps) tabName = "MyMaps";
    else if (index=== sidebar_tabIndexValue.themes) tabName = "Themes";
    else if (index=== sidebar_tabIndexValue.reports) tabName = "Reports";
    return tabName;
  }
  ...
  activateTab(tabName) {
    let id= this.getTabIndex(tabName);
    if(id !==-1) {
      // SET SELECTED TAB
  ...
  onTabSelect = tabIndex => {
    ...
    let name = this.getTabName(tabIndex);
    helpers.addAppStat("Tab", name);
  ...
  getRenderContent () {
    let context=null;
    if(window.appConfig.BLI_Under_Dev.Dev_MenuOnly){
      context =(
        <SidebarComponent
        touch={false}
        id="sc-sidebar"
        ...
        sidebar={
          <React.Fragment>
            <Tabs forceRenderTabPanel={true} selectedIndex={this.state.tabIndex} onSelect={this.onTabSelect}>
              <TabList>
                {/**BLI:Tab header icon field */}
                <Tab id="tab-layers">
                  <TabButton imageURL={images["legend-32x32.png"]} name="Layers" />
                </Tab>
                <Tab id="tab-mapview">
                  <TabButton imageURL={images["myview.jpg"]} name="mapview" />
                </Tab>
               {/* <Tab id="tab-mymaps">
            ...
            {/**BLI:the following part is to Draw the min side bar */}
           <SidebarSlim
