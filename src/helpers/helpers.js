...
export function addAppStat(type, description) {
  if (window.appConfig.includeAppStats === false) return;

  // IGNORE LOCAL HOST DEV
  if (window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1") return;

  const appStatsTemplate = (type, description) => `${window.appConfig.appStatsUrl}opengis/${type}/${description}`;
...
// SHOW MESSAGE
//BLI: change waiting time from 2000 to 20000
export function showMessage(title = "Info", messageText = "Message", color = "green", timeout = 20000) {
...
export function bufferGeometry(geometry, distanceMeters, callback) {
  const url = window.appConfig.apiUrl + "postBufferGeometry/";
...
export function getGeometryCenter(geometry, callback) {
  const url = window.appConfig.apiUrl + "postGetGeometryCenter/";
