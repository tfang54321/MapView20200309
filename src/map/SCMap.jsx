...
//BLI: it looks like the WMS url string template
const feedbackTemplate = (xmin, xmax, ymin, ymax, centerx, centery, scale) =>
  `${window.appConfig.feedbackUrl}/?xmin=${xmin}&xmax=${xmax}&ymin=${ymin}&ymax=${ymax}&centerx=${centerx}&centery=${centery}&scale=${scale}&REPORT_PROBLEM=True`;
...
class SCMap extends Component {
  ...
  componentDidMount() {
    let centerCoords = window.appConfig.centerCoords;
    ...
    if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)) {
      ...
      helpers.showURLWindow(window.appConfig.ieWarningUrl);
    }
  }
  ...
  getRenderContent =()=>{
    ...
    if(window.appConfig.BLI_Under_Dev.Dev_SimpleViewMap_only) {
      context=(
          ...
          <Navigation />
          {/**BLI: footerTools to display scale */}
          <FooterTools />
          <BasemapSwitcher />
        </div>
      );
