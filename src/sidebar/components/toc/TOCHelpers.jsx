...
export function disableLayersVisiblity(layers, callback) {
  ...
  for (let index = 0; index < layers.length; index++) {
    ...
    //BLI: crash here because there is no openlayers object created.
    if(!window.appConfig.enable_AddNewMap) {
      //BLI:Orig code should not be used inf the future;
      if(layer && layer.layer&& layer.layer !==undefined)
         layer.layer.setVisible(false);
    } 
    else {
      if(layer&& layer.visible && layer.layer&& layer.layer !==undefined){
...
export function enableLayersVisiblity(layers, callback) {
  ...
  for (let index = 0; index < layers.length; index++) {
    const layer = layers[index];
    if(layer.visible && layer.layer &&layer.layer !==undefined){
       layer.layer.setVisible(layer.visible);//turn off the ol layer
    } 
    let newLayer = Object.assign({}, layer);
...
export function resetLayerDefaults(layers, callback) {
  ...
  for (let index = 0; index < layers.length; index++) {
    ...
    if(!window.appConfig.enable_AddNewMap) {
      ...
      if(layer.visible && layer.layer !==undefined){
          layer.layer.setVisible(layer.visible);//turn off the ol layer
      }
    } 
    let newLayer = Object.assign({}, layer);
...
export function updateLayerIndex(layers, callback) {
  ...
  for (let index = 0; index < layers.length; index++) {
    ...
    let newLayer = Object.assign({}, layer);
    //BLI: the feature layer will not be created for openlayers layer object.
    if(newLayer.layer) {
