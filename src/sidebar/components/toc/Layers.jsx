...
//BLI:sort by drawindex(display priority)
  ...
  onCheckboxChange = layerInfo => {
    ...
    if(layerInfo.layer&&layerInfo.layer!==undefined){
    ...
    else if(layerInfo.parentLayer)
    {
        ...
        //Debugging purpose:
        window.debuggingLog(layerList);
        //
        if(!topLayer.addedToMap) {
          topLayer.addedToMap = true;
          window.map.addLayer(topLayer.olLayer.olLayer);
          //parent.layer.setZIndex(1);
        }
        topLayer.olLayer.olLayer.setVisible((layerList.length===0)?false:true);
        window.map.render();

        // let params1 = source.getParams();
        // params1.t = new Date().getMilliseconds();
        // source.updateParams(params1);
        // window.map.render();
    }
