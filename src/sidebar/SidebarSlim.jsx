...
const feedbackTemplate = (xmin, xmax, ymin, ymax, centerx, centery, scale) =>
  `${window.appConfig.feedbackUrl}/?xmin=${xmin}&xmax=${xmax}&ymin=${ymin}&ymax=${ymax}&centerx=${centerx}&centery=${centery}&scale=${scale}`;
...
const SidebarSlim = props => {
  if(window.appConfig.BLI_Under_Dev.Dev_MenuOnly)
  {
    return (
      <div className={window.sidebarOpen ? "sc-hidden" : "sc-sidebar-slim-container"}>
        ...
        <div className="sc-sidebar-slim-button-container" onClick={() => props.onClick("mapview")}>
          <button className="sc-sidebar-slim-button">
            <img src={images["myview.jpg"]} alt="mapview" />
            <br />
            <span >mapview</span>
          </button>
        </div>
  ...
  else { //orginal code //need to remove in the future
      return (
        <div className={window.sidebarOpen ? "sc-hidden" : "sc-sidebar-slim-container"}>
          <div className="sc-sidebar-slim-button-container" onClick={() => props.onClick("layers")}>
            <button className="sc-sidebar-slim-button">
