...
class TOC extends Component {
  ...
  loadWMSLayer = callback => {
    //BLI:This is used to load the wms layers based on the specified wms url.
     sessionStorage.removeItem(this.storageMapDefaultsKey); 
      ...
      //BLI: Call fetch to request the WMS GetCapability based on the TOCConfig.json file
  ...
  loadSymbalOn=()=>{
    ...
    const loadingSym = (
      <div className= "sc-toc-main-container-loading">
        <img className="sc-toc-loading" src={images["loading.gif"]} alt="loading" />
      </div>
    );
    ReactDOM.render(loadingSym, document.getElementById("portal-root"));
  };
  //BLI: show loading symbole
  loadSymbalOff=()=>{
    if(this.state.isLoading) {
    ...
    const loadingSym = (
      <div className= "sc-toc-main-container-loading sc-hidden">
        <img className="sc-toc-loading" src={images["loading.gif"]} alt="loading" />
      </div>
    );
    ReactDOM.render(loadingSym, document.getElementById("portal-root"));
    ...
    if(window.appConfig.enable_AddNewMap) {
      content = (
        ...
        <div className={this.state.isLoading ? "sc-toc-main-container-loading" : "sc-toc-main-container-loading sc-hidden"}>
          <img className="sc-toc-loading" src={images["loading.gif"]} alt="loading" opacity="0.5" />
        </div>
